...
- `js/sync-collections.js`：按记录 ID 同步待办、笔记、链接、日历事件、习惯和教材；每条记录使用独立云端行，排序另存一行。
...
六组有稳定记录 ID 的数组使用 `mst:item:v1:<集合>:<记录 ID>` 云端行。不同记录可独立合并；同一记录同时修改、同时调整排序或旧设备继续改写整组数据时，设置页显示冲突供用户选择。删除使用记录墓碑；旧版整组云端行只作为首次迁移来源。计时记录和待办完成日志暂沿用原同步通道，因为其旧记录 ID 不保证跨设备唯一。多端模拟脚本为 `tests/probe/multidevice-sync.js`，在两套隔离客户端与共享内存云表上验证迁移、并发、删除、排序和冲突处理。

## 验证

AI 请求固定使用发起时的模型配置、联网模式和对话 ID，后台生成与停止互不串用；恢复标记按对话保存且不含凭据。默认超时为 60 秒，重试 2 次，停止也会取消退避等待。官方 OpenAI/DeepSeek 端点优先使用原生 Function Calling，其他端点保留文本/DSML 兼容。工具调用经过 Schema 预检、权限分级、持久化确认和结构化回执；连续只读操作可限流并行，写操作按顺序执行并在同轮失败时回滚。跨刷新幂等账本防止重复写入，不自动重试工具写操作；失败、未执行、回滚和轮次耗尽均会显式报告。

历史上下文按完整问答轮次保留，较早内容使用本地摘录，不额外调用模型。每个 Key 可配置上下文预算（默认估算 32768 Token，含系统提示和输出预留）；超大当前输入会提示调整，原始对话不被截断保存。一次性提醒默认使用具体本地日期，设置中可查看和修改创建依据；只有用户要求每天提醒时才使用每日模式。
