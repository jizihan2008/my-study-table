...
create table if not exists public.profiles (
  -- 不直接外键引用 auth.users：不同 CloudBase 环境的 auth.users.id 可能是 uuid 或 varchar。
  -- 登录用户 ID 统一以字符串保存，访问权限仍由 auth.uid()::text + RLS 严格控制。
...
create table if not exists public.friendships (
  id uuid primary key default gen_random_uuid(),
  user_id varchar(64) not null references public.profiles(id) on delete cascade,
  friend_id varchar(64) not null references public.profiles(id) on delete cascade,
...
create table if not exists public.conversations (
  id uuid primary key default gen_random_uuid(),
  user_a varchar(64) not null references public.profiles(id) on delete cascade,
  user_b varchar(64) not null references public.profiles(id) on delete cascade,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.messages (
  ...
  sender_id varchar(64) not null references public.profiles(id) on delete cascade,
...
as $$
  select exists (
    ...
    where f.status = 'accepted'
      and ((f.user_id = auth.uid()::text and f.friend_id = target)
        or (f.user_id = target and f.friend_id = auth.uid()::text))
  );
$$;
...
begin
  execute $policy_sql$
    create policy "friendships_readable" on public.friendships
      for select to authenticated using (auth.uid()::text = user_id or auth.uid()::text = friend_id);
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "friendships_update" on public.friendships
      for update to authenticated using (auth.uid()::text = user_id or auth.uid()::text = friend_id)
...
begin
  execute $policy_sql$
    create policy "friendships_delete" on public.friendships
      for delete to authenticated using (auth.uid()::text = user_id or auth.uid()::text = friend_id);
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "stats_readable" on public.study_stats
      for select to authenticated using (auth.uid()::text = user_id or public.is_friend(user_id));
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "conv_readable" on public.conversations
      for select to authenticated using (auth.uid()::text = user_a or auth.uid()::text = user_b);
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "conv_insert" on public.conversations
      for insert to authenticated with check (auth.uid()::text = user_a or auth.uid()::text = user_b);
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "conv_update" on public.conversations
      for update to authenticated using (auth.uid()::text = user_a or auth.uid()::text = user_b)
    ...
    create policy "messages_readable" on public.messages
      for select to authenticated using (
        exists (
          ...
          where c.id = conversation_id and (c.user_a = auth.uid()::text or c.user_b = auth.uid()::text)
        )
      );
  $policy_sql$;
exception
...
-- ═══════════════════════════════════════════════════════════════════
-- 授权 Data API 角色。私有业务表只授予 authenticated，公开视图/市场列表才授予 anon。
-- 若新建项目时关闭了 "Automatically expose new tables"，此段必不可少；
-- 若保持开启，重复 GRANT 也幂等无害。数据访问仍由上方 RLS 策略精确控制。
-- ═══════════════════════════════════════════════════════════════════
grant usage on schema public to anon, authenticated, service_role;
revoke all on table public.profiles, public.friend_groups, public.friendships, public.study_stats, public.activities from anon;
grant all on table public.profiles to authenticated, service_role;
grant all on table public.friend_groups to authenticated, service_role;
grant all on table public.friendships to authenticated, service_role;
grant all on table public.study_stats to authenticated, service_role;
grant all on table public.activities to authenticated, service_role;
...
create table if not exists public.plugin_store_items (
  id         uuid primary key default gen_random_uuid(),
  author_id  varchar(64) references public.profiles(id) on delete set null,
...
begin
  execute $policy_sql$
    create policy "Author can insert pending plugins" on public.plugin_store_items
      for insert to authenticated with check (author_id = auth.uid()::text and status = 'pending');
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "Author can delete own plugins" on public.plugin_store_items
      for delete to authenticated using (author_id = auth.uid()::text);
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
begin
  execute $policy_sql$
    create policy "Anyone can read plugin files" on storage.objects
      for select to anon, authenticated using (bucket_id = 'plugin-store');
  $policy_sql$;
exception
  when duplicate_object then null;
end
...
-- 授权 Data API 角色
revoke all on table public.user_sync_items from anon;
grant all on table public.user_sync_items to authenticated, service_role;
...
as $$
  ...
  where item.user_id = auth.uid()::text
  group by item.kind;
$$;
...
-- ═══════════════════════════════════════════════════════════════════
-- 1. bucket：plugin-store，Public = true
-- 2. Storage RLS Policy → plugin-store：
--    SELECT (读取)：bucket_id = 'plugin-store' → 允许所有人（public）
--    INSERT/UPDATE/DELETE：仅认证用户可操作以自己 user_id 开头的路径
-- 3. 插件文件存储格式：
--    - 路径：<user_id>/<ext_id>/<version>/<ext_id>.zip
--    - zip 内容：manifest.json + main.js
--    示例：user-id/my-plugin/1.0.0/my-plugin.zip
