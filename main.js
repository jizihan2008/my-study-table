...
// ═══════════════════════════════════════════════
//  本地 WebRTC 信令服务器（纯局域网，替代依赖 Supabase Realtime 的信令）
//  用于桌面端「发送到手机」教材 PDF 传输：信令不再依赖不稳定的云端 Realtime，
//  而是桌面端起一个本地 HTTP 服务器作为信令中继，手机端通过局域网 IP 轮询交换。
//  端点：
//    POST /signal   body: { code, event, payload }   提交一条信令（手机→桌面 或 桌面→手机）
//    GET  /signal/<code>?since=<seq>                 拉取自 seq 之后的新信令（增量轮询）
//    GET  /ping                                      健康检查
// ═══════════════════════════════════════════════
const signalServerState = { server: null, port: 0, startedAt: 0 };
// 每个 code 维护一个信令队列：{ [code]: [ { seq, event, payload } ] }
const signalQueues = new Map();
let signalSeqCounter = 0;

function getLanIps() {
  const ips = [];
  // 仅保留真实私有局域网地址，过滤代理/VPN 虚拟网卡（如 198.18.0.1 是 Clash 虚拟网卡）
  const isPrivate = (ip) => {
    const p = String(ip || '').split('.').map(Number);
    if (p.length !== 4 || p.some(isNaN)) return false;
    if (p[0] === 10) return true;                                    // 10.x.x.x
    if (p[0] === 172 && p[1] >= 16 && p[1] <= 31) return true;        // 172.16-31.x.x
    if (p[0] === 192 && p[1] === 168) return true;                    // 192.168.x.x
    if (p[0] === 169 && p[1] === 254) return true;                    // 169.254 APIPA
    return false;
  };
  const skipName = (name) => {
    const ln = String(name || '').toLowerCase();
    return ln.includes('proxy') || ln.includes('virtual') || ln.includes('vmware') ||
      ln.includes('loopback') || ln.includes('docker') || ln.includes('tun') ||
      ln.includes('tap') || ln.includes('wintun') || ln.includes('clash') ||
      ln.includes('zerotier') || ln.includes('utun') || ln.includes('vpn');
  };
  try {
    const os = require('os');
    const nifs = os.networkInterfaces();
    for (const name of Object.keys(nifs)) {
      if (skipName(name)) continue;                                  // 跳过虚拟/代理网卡
      for (const ni of nifs[name] || []) {
        if (ni.family === 'IPv4' && !ni.internal && isPrivate(ni.address)) ips.push(ni.address);
      }
    }
  } catch (e) {}
  // 若过滤后为空（极端网络），退回返回全部非内网 IPv4（排除 198.18.0.1）
  if (ips.length === 0) {
    try {
      const os = require('os');
      const nifs = os.networkInterfaces();
      for (const name of Object.keys(nifs)) {
        for (const ni of nifs[name] || []) {
          if (ni.family === 'IPv4' && !ni.internal && ni.address !== '198.18.0.1') ips.push(ni.address);
        }
      }
    } catch (e) {}
  }
...
function readJsonBody(req) {
  return new Promise((resolve) => {
    let body = '';
    req.on('data', (d) => { body += d; if (body.length > 2 * 1024 * 1024) { req.destroy(); resolve(null); } });
    req.on('end', () => { try { resolve(body ? JSON.parse(body) : null); } catch (e) { resolve(null); } });
    req.on('error', () => resolve(null));
  });
}

function signalHttpHandler(req, res) {
  // CORS：允许手机端（任意 origin）跨域访问
  ...
  if (req.method === 'POST' && url.startsWith('/signal')) {
    readJsonBody(req).then((body) => {
      if (!body || !body.code || !body.event) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, reason: 'bad request' }));
        return;
      }
      const code = String(body.code).replace(/[^\w-]/g, '').slice(0, 16);
      const seq = ++signalSeqCounter;
      const entry = { seq, event: String(body.event), payload: body.payload || {}, from: body.from === 'send' ? 'send' : 'recv' };
      if (!signalQueues.has(code)) signalQueues.set(code, []);
      const q = signalQueues.get(code);
      q.push(entry);
      if (q.length > 500) q.splice(0, q.length - 500);   // 防止内存无限增长
      // 推送信令给渲染进程（发送端监听 incoming 事件处理手机端的 ready/answer/ice）
      try { sendToRenderer('webrtc:signal-incoming', { code, event: entry.event, payload: entry.payload }); } catch (e) {}
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, seq }));
    });
    return;
  }
  // GET /signal/<code>?since=<seq>
  if (req.method === 'GET' && url.startsWith('/signal/')) {
    const rest = url.split('?')[0];
    const code = rest.slice('/signal/'.length).replace(/[^\w-]/g, '').slice(0, 16);
    const since = Number((url.split('since=')[1] || '0').split('&')[0]) || 0;
    const q = signalQueues.get(code) || [];
    // 只返回「桌面端（发送端）发」的信令（offer/ice），过滤掉接收端自己 POST 的信令（ready/answer/ice），
    // 否则接收端轮询会把自己发的信令拉回来重复处理。
    const items = q.filter((x) => x.seq > since && x.from === 'send');
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, items, lastSeq: q.length ? q[q.length - 1].seq : since }));
    return;
  }
  // GET /ping
...
// IPC: 启动本地信令服务器（返回端口 + 局域网 IP）
ipcMain.handle('webrtc:signal-start', async (event, preferredPort) => {
  try {
    if (signalServerState.server) {
      return { ok: true, port: signalServerState.port, ips: getLanIps(), alreadyRunning: true };
    }
    const port = Number(preferredPort) || 0;
    const server = http.createServer(signalHttpHandler);
    await new Promise((resolve, reject) => {
      server.once('error', reject);
      server.listen(port, '0.0.0.0', () => resolve());
    });
    signalServerState.server = server;
    signalServerState.port = server.address().port;
    signalServerState.startedAt = Date.now();
    return { ok: true, port: signalServerState.port, ips: getLanIps() };
  } catch (e) {
    return { ok: false, reason: String((e && e.message) || e) };
  }
});

// IPC: 查询信令服务器状态（端口 + 局域网 IP）
ipcMain.handle('webrtc:signal-state', async () => {
  return {
    ok: !!signalServerState.server,
    port: signalServerState.port,
    ips: getLanIps()
  };
});
...
ipcMain.handle('webrtc:signal-stop', async () => {
  try {
    if (signalServerState.server) {
      await new Promise((resolve) => signalServerState.server.close(() => resolve()));
      signalServerState.server = null;
      signalServerState.port = 0;
    }
    return { ok: true };
  } catch (e) {
    return { ok: false, reason: String((e && e.message) || e) };
